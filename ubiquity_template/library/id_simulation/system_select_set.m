...
% % following:
%
% cfg = system_select_set(cfg, 'mouse');
%
...
%
% cfg = system_select_set(cfg, 'mouse', {'Vp', 'CL'});
%
...
% Clearing things out
cfg.estimation.to_estimate = struct();
cfg.estimation.set_name    = struct();
cfg.estimation.mi          = struct();
cfg.estimation.parameters  = struct();
...
%
% Applying the IIV sets
%
if(~isempty(fieldnames(cfg.iiv)))
  if(isfield(cfg.iiv.sets, set_name))
    iiv_set_name = set_name;
  else  
    iiv_set_name = 'default';
  end
  iiv_set_data        = getfield(cfg.iiv.sets, iiv_set_name);
  cfg.options.mi.iivs = getfield(cfg.options.mi.iiv.sets, iiv_set_name);
  cfg.iiv.current_set = iiv_set_name;
  cfg.iiv.iivs        = iiv_set_data.iivs;
  cfg.iiv.parameters  = iiv_set_data.parameters;
  cfg.iiv.values      = iiv_set_data.values;
end
